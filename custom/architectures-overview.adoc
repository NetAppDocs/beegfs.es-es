...
BeeGFS se versión siguiendo un esquema de control de versiones <major>.<minor>.<patch> y la colección mantiene roles para cada versión compatible de <major>.<minor> de BeeGFS, por ejemplo BeeGFS 7.2 o BeeGFS 7.3. A medida que se publican actualizaciones de la colección, la versión de revisión de cada rol se actualizará para señalar la última versión disponible de BeeGFS para esa rama de versión (por ejemplo: 7.2.8). Cada versión de la colección también está probada y es compatible con distribuciones y versiones de Linux específicas, actualmente Red Hat para nodos de archivos y Red Hat y Ubuntu para clientes. No se admite la ejecución de otras distribuciones; no se recomienda ejecutar otras versiones (especialmente otras versiones principales).
...
Los nodos de archivo deben ejecutar Red Hat Enterprise Linux (RHEL) 9.4 y tener acceso al repositorio de alta disponibilidad (HA) que contiene los paquetes necesarios (pacemaker, corosync, fencing-agents-all y resource-agents). Por ejemplo, se puede ejecutar el siguiente comando para habilitar el repositorio correspondiente en RHEL 9:
...
Hay disponible un rol de Ansible para el cliente BeeGFS para instalar el paquete de cliente BeeGFS y gestionar los montajes BeeGFS. Esta función se ha probado con RHEL 9.4 y Ubuntu 22.04.

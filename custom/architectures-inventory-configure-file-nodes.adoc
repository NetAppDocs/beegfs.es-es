...
Haz clic en link:https://github.com/NetApp/beegfs/blob/master/getting_started/beegfs_on_netapp/gen2/H2_DAC_IB_A2_EF600/beegfs_cluster_building_blocks/beegfs_ha_building_block_cluster_1/host_vars/beegfs_01.yml["aquí"^] para ver un ejemplo de un archivo de inventario completo que representa un único nodo de archivo.
...
Los adaptadores NVIDIA ConnectX-Virtual Protocol Interconnect® (VPI) admiten tanto InfiniBand como Ethernet como capa de transporte. El cambio entre modos no se negocia automáticamente y debe configurarse usando la herramienta `mstconfig` incluida en `mstflint`, un paquete de código abierto que forma parte de link:https://network.nvidia.com/products/adapter-software/firmware-tools/["Herramientas de Firmare de NVIDIA (MFT)"^]. Cambiar el modo de los adaptadores solo es necesario hacerlo una vez. Esto se puede hacer manualmente o incluirlo en el inventario de Ansible como parte de cualquier interfaz configurada usando la sección  `eseries-[ib|ib_iser|ipoib|nvme_ib|nvme_roce|roce]_interfaces:` del inventario, para que se compruebe o aplique automáticamente.

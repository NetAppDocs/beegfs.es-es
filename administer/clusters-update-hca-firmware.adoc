...
TIP: Para los adaptadores Lenovo ConnectX-7, utilice la `mlxfwmanager_LES` herramienta, que está disponible en la página de NVIDIAlink:https://network.nvidia.com/support/firmware/lenovo-intelligent-cluster/["Firmware de OEM"^].
...
NOTE: Es posible que algunas actualizaciones de firmware requieran un reinicio para aplicar la actualización. Consulta link:https://networking-docs.nvidia.com/mftswum/4201/mlxfwreset-loading-firmware-on-5th-generation-devices-tool#id-(4.20.1)mlxfwreset–LoadingFirmwareon5thGenerationDevicesTool-mlxfwresetLimitations["Limitaciones de mlxfwreset de NVIDIA"^] para obtener orientación. Si se requiere un reinicio, haz un reinicio en lugar de restablecer los adaptadores.
...
----
.. Reiniciar el  `eseries_nvme_ib.service` .
+
[source, console]
----

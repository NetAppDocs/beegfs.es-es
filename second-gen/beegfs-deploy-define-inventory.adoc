...
NOTE: El tamaño del volumen se especifica como un porcentaje del grupo de almacenamiento global (también denominado grupo de volúmenes). NetApp recomienda encarecidamente que dejes algo de capacidad libre en cada grupo para permitir el sobreaprovisionamiento de los SSD (para más información, consulta https://www.netapp.com/pdf.html?item=/media/17009-tr4800.pdf["Introducción a la cabina EF600 de NetApp"^]). El grupo de almacenamiento, `beegfs_m1_m2_m5_m6`, también asigna un 1 % de la capacidad del grupo al servicio de gestión. Así que, para los volúmenes de metadatos del grupo de almacenamiento, `beegfs_m1_m2_m5_m6`, cuando se usen unidades de 1,92 TB o 3,84 TB, pon este valor en `21.25`; para unidades de 7,65 TB, pon este valor en `22.25`; y para unidades de 15,3 TB, pon este valor en `23.75`.
...
NOTE: Para conocer el tamaño correcto a utilizar, consulte link:beegfs-deploy-recommended-volume-percentages.html["Se recomendaron porcentajes de sobreaprovisionamiento del pool de almacenamiento"] ..

...
. Conecta los nodos block y file usando el link:beegfs-technology-requirements.html#file-node-requirements["1M cables de cobre de conexión directa InfiniBand HDR 200GB"], para que coincidan con la topología que se muestra en la siguiente figura.
+
...
. Conecta los puertos restantes de InfiniBand en el nodo de archivos al switch de InfiniBand de la red de almacenamiento usando el link:beegfs-technology-requirements.html#storage-network-switch-requirements["2m cables InfiniBand"] específico para tu switch de almacenamiento InfiniBand.
+
